...
class MazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        actions_fn = [self.down, self.up, self.left, self.right, self.eat]
        if not isinstance(action, int):
            action = action.item()
        if action < 0 or action > 4:
            exit('Invalid action.')
        done, self.last_r = actions_fn[action]()
        if done:
    ...
    def eat(self):
        """
        Execute the action "eat" in the environment.
        :return: true of the state is terminal and false otherwise.
        """
    ...
    def is_terminal_state(self):
        ...
        """
        return True if self.y_pos == self.end_state[0] and self.x_pos == self.end_state[1] else False
